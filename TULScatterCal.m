...
%     [pscat,sscat]=polyfit(Ul(counter1,:),TheorIntens,nscat); % fits data to polynomial
    ...
    text((max(xiscat)-min(xiscat))/2,CalCurve((max(xiscat)-min(xiscat))/2)...
        ,sprintf('%+5.3e*X',CalCurve.a),'FontSize',10)% print the equation on the figure
%     text((max(xiscat)-min(xiscat))/2,CalCurve((max(xiscat)-min(xiscat))/2)...
%         ,sprintf('%+5.3e*X + %+5.3e',CalCurve.a,CalCurve.b),'FontSize',10)% print the equation on the figure
