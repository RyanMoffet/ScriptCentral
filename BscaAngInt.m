function [B]=BscaAngInt(sizdist,insiz,wavelen,SpecResp,nc,ns,kc,ks,VolFrac,rho,apssiz,CalcType)
...
%% insiz is the x-axis of the size distribution (in micron)
            ...
            for k=1:length(sizdist(1,:))
                if isnan(sizdist(j,k)) | SpecResp(i) == 0
                    ...
                    Kernel(i,k)=SpecResp(i)*sizdist(j,k)*...
                        MieAngFun(1,nc,kc,wavelen(i)/1000,siz(k),la,ua,1)...  
                        *1/siz(k)*10^12*(3/2);  
                end
            end
        end
        ...
        for i=1:length(wavelen)
            for k=1:length(sizdist(1,:))
                ...
                if isnan(sizdist(j,k)) | SpecResp(i) == 0
                    ...
                    Kernel(i,k)=SpecResp(i)*sizdist(j,k)*...
                        CotAngFun(1,nc,kc,ns,ks,wavelen(i)/1000,Dc(k),Dp(k),la,ua,1)...  
                        *1/siz(k)*10^12*(3/2);
                end
            end
        end
