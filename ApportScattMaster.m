...
% lumprange{1}=[.1,1.7;1.96,3];
% lumprange{2}=[.1,1.7;1.96,3];
% lumprange{3}=[.1,1.7];
% lumprange{4}=[.1,1.7];

% lumpstr={'Sea Salt','Dust','Carbon','EC'};
% for i=1:length(lumpstr)
%     [LumpProcA{i},LumpRawA{i}]=ProcMsb(lumptype{i},SaaSlope,SaaInt...
%          ,'saarea',lumprange{i},.05)
...
%          ,'sbarea',lumprange{i},.05)
%     title(sprintf('CALCOFI lumptype %g, %s PMT B',i,lumpstr{i}))
%     FileName = sprintf('ScattA_Lump%g.fig',i); % Names the file (differently) for each cluster
%     saveas (gcf,FileName, 'fig');
%     close
%     save LumpWorkup.mat
%     [LumpProcAB{i}]=ABProcErr(LumpProcA{i},LumpProcB{i})
%     jobcount=jobcount+1
% end
% 
% 
% %% Fit SS
% 
% [LumpER{1},LumpSqErr{1},LumpTD{1},Lumpr2{2}]=ParamFit1(LumpProcAB{1},0.1,1.5,[1.33,1.4,0,0.001,0.7,1.5],3)
% 
% %% Fit Dust
% 
% [LumpER{2},LumpSqErr{2},LumpTD{2},Lumpr2{2}]=ParamFit1(LumpProcAB{2},0.1,1.5,[1.33,1.4,0,0.001,0.7,1.5],3)
% 
% %% Fit Carbon
% 
% [LumpER{3},LumpSqErr{3},LumpTD{3},Lumpr2{3}]=ParamFit1(LumpProcAB{3},0.1,1.5,[1.33,1.5,0,0.001,0.7,1.7],3)
% 
% %% Fit EC
% 
% [LumpER{4},LumpSqErr{4},LumpTD{4},Lumpr2{4}]=CoatParamFit1(LumpProcAB{4},0.1,1.5,[1.7,0.5,1.33,0,0.01,1],0)
% 
% save LumpWorkup.mat
