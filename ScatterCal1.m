...
function [Slpe,Int]=ScatterCal1(InId,PslSizes,SpeedMode)
...
for counter1=1:length(intensarray)
    ...
    for i = 1:length(PslSizes)
        ...
        subplot(3,3,i),hist(intens,NumBin) % these next few lines make the intensity histogram plots
    ...
    for counter2=1:length(PslSizes)
        TheorIntens(counter2)=MieCalF(1.0,nimp,kimp,lambdaimp,PslSizes(counter2),1)-...
                      MieCalF(1.0,nimp,kimp,lambdaimp,PslSizes(counter2),2);
    end
    nscat=1; % order of polynomial to be fit
    [pscat,sscat]=polyfit(Av(counter1,:),TheorIntens,nscat); % fits data to polynomial
    xiscat=linspace(min(Av(counter1,:)),max(Av(counter1,:)),100); % creates data input for the polynomial evaluation
    zzscat=polyval(pscat,xiscat); % evaluate the polynomial
    Output = polyval(pscat,Av(counter1,:));
    Corrolation = corrcoef(TheorIntens', Output');
    rsq=Corrolation(1,2)^2;
    figure,plot(Av(counter1,:),TheorIntens,'o',xiscat,zzscat)
    text((max(xiscat)-min(xiscat))/2,polyval(pscat,(max(xiscat)-min(xiscat))/2)...
        ,sprintf('%+5.3e*X%+5.3e',pscat),'FontSize',10)% print the equation on the figure
    text((max(xiscat)-min(xiscat))/3,polyval(pscat,(max(xiscat)-min(xiscat))/3)...
    ...
    set(gca, 'FontSize', 6)  
    Slpe(counter1)=pscat(1);
    Int(counter1)=pscat(2);
end
