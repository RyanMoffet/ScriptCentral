...
DATA(19).varstr = {'rh'};  %% search strings
DATA(19).range = [0 100];
DATA(19).ylabel = {'relative humidity (%)'};
DATA(19).title = getCDFlongname(DATA(19), PAR(1));

f1 = figure('Position', figrect, 'Color', 'white', 'PaperPosition', [0.5 0.5 7.5 10.5]);
X = [16:19];  %%% range of cells to plot
for i=1:length(X)
    subplot(length(X),1,i); h1 = CDFplot(timematrix, DATA(X(i)), PAR(1));
    text(min(get(gca, 'Xlim')), max(get(gca, 'Ylim')), cellstr(DATA(X(i)).title), ...
        'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left', 'Interpreter', 'none');
...
f6 = figure('Position', figrect, 'Color', 'white', 'PaperPosition', [0.5 0.5 7.5 10.5]);
...
%%%% per liter
DATA(15).varstr = {'concic', 'conctotc'};  %% search strings
DATA(15).range = [0];
DATA(15).ylabel = {'per liter (L^{-1})'};

f2 = figure('Position', figrect, 'Color', 'white', 'PaperPosition', [0.5 0.5 7.5 10.5]);
...
%%%%%%%%%%%%% Cloud and water %%%%%%%%%%%%%%%%%%%%
DATA(8).varstr = {'pvmlwc', 'PLWCX_OBR', 'lwcfocp', 'PLWCF_IBL', 'jlb_lwc2_IBL', 'lwc2dc', 'lwc2dcm', 'lwc100', 'rlwc'};  %%% liquid water
DATA(8).range = [0 1];
DATA(8).ylabel = {'liquid water content (g m^{-3})'};

DATA(9).varstr = {'iwcc'};  %% ice water
DATA(9).range = [0 0.5];
DATA(9).ylabel = {'ice water content (g m^{-3})'};

DATA(10).varstr = {'rwcc'};  %%% rain water
DATA(10).range = [0 1];
...
f3 = figure('Position', figrect, 'Color', 'white', 'PaperPosition', [0.5 0.5 7.5 10.5]);
...
f4 = figure('Position', figrect, 'Color', 'white', 'PaperPosition', [0.5 0.5 7.5 10.5]);
